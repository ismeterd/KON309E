...
// * Fun part additional to Experiment 3 - Part 3.
...
void pint_intr2_callback(pint_pin_int_t pintr, uint32_t pmatch_status);
void pint_intr3_callback(pint_pin_int_t pintr, uint32_t pmatch_status);
...
int timer[9] = {10, 20, 30, 40, 50, 75, 100, 200, 500};
...
//    PINT_Init(PINT);
    ...
    SYSCON_AttachSignal(SYSCON, kPINT_PinInt0, kSYSCON_GpioPort0Pin8ToPintsel);
    PINT_PinInterruptConfig(PINT,                // Pin INT base register address
                            kPINT_PinInt0,       // Use Pin INT 2
                            kPINT_PinIntEnableFallEdge, // At falling edge.
                            pint_intr2_callback); // Name of the callback function
    PINT_EnableCallbackByIndex(PINT, kPINT_PinInt0);


    SYSCON_AttachSignal(SYSCON, kPINT_PinInt2, kSYSCON_GpioPort0Pin7ToPintsel);
    PINT_PinInterruptConfig(PINT,                // Pin INT base register address
                            kPINT_PinInt2,       // Use Pin INT 3
                            kPINT_PinIntEnableFallEdge, // At falling edge.
                            pint_intr3_callback); // Name of the callback function
...
{
    PRINTF("\033[0m");
    PRINTF("\n[PIN INTERRUPT]: PINT Pin Interrupt 1 event detected.\r\n");
    brightnessCounter++;
...
{
    PRINTF("\033[0m");
    PRINTF("\n[PIN INTERRUPT]: PINT Pin Interrupt 2 event detected.\r\n");
    timerCounter--;
    if (timerCounter == -1)
    {
        timerCounter = 0;
        PRINTF("\n\t[WARNING]: Cycle time cannot be lower than 100 ms!\r\n");
    }
    else
    {
        PRINTF("[PIN INTERRUPT]:\t Cycle Time has changed -> time: %d ms \r\n", timer[timerCounter] * 10);
    }
}
...
{
    PRINTF("\033[0m");
    PRINTF("\n[PIN INTERRUPT]: PINT Pin Interrupt 3 event detected.\r\n");
    timerCounter++;
    if (timerCounter == 9)
    {
        timerCounter = 8;
...
void MRT0_IRQHandler(void)
{
    ...
    { // This flag is set by the MRT ISR.
        counter++;
        // timer[timerCounter] -> default = 50, so 500ms
        if (counter > timer[timerCounter])
        {
            counter = 0;
        }
        int cycle_time = timer[timerCounter];
